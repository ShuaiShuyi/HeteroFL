...
class Federation:
    def __init__(self, global_parameters, rate, label_split):
        ...
        # print("label_split:",self.label_split)
    ...
    def split_model(self, user_idx):
        if cfg['model_name'] == 'conv':
            ...
            idx = [OrderedDict() for _ in range(len(user_idx))]     # [OrderedDict(),OrderedDict()...]
                                                                                                        # glabal_parameters为字典，由model.state_dict()得到
            output_weight_name = [k for k in self.global_parameters.keys() if 'weight' in k][-1]        # 键是层的名称（例如，fc.weight 或 fc.bias），值是对应的参数（tensor)
            output_bias_name = [k for k in self.global_parameters.keys() if 'bias' in k][-1]            # 这里的output_xx是[-1]，也就是最后一个（输出层）
    ...
    def white(self, messages, byzantinesize, device):
        # 均值相同，方差较大
        mu = torch.mean(messages[0:-byzantinesize], dim=0)
        messages[-byzantinesize:].copy_(mu)
        
        # noise = torch.randn((byzantinesize, messages.size(1)), dtype=torch.float64,device=device)
        noise = torch.randn_like(messages[-byzantinesize:], dtype=torch.float64,device=device)
        messages[-byzantinesize:].add_(noise)
        
    def maxValue(self, messages, byzantinesize):
        mu = torch.mean(messages[0:-byzantinesize], dim=0)
        meliciousMessage = -3*mu
        messages[-byzantinesize:].copy_(meliciousMessage)
        
    def zeroGradient(self, messages, byzantinesize):
    ...
    def combine(self, local_parameters, param_idx, user_idx):
        count = OrderedDict()
        attack = False
        byzantinesize = int(np.ceil(cfg['byzantine_frac'] * len(local_parameters)))
        if cfg['model_name'] == 'conv':
            ...
            for k, v in self.global_parameters.items():
                ...
                if attack:
                    ...
                    if tmp_v.shape == v.shape:
                        ...
                        self.white(difference,byzantinesize,v.device)            # white
                        # self.maxValue(difference,byzantinesize)         # maxvalue
                        # self.zeroGradient(difference,byzantinesize)     # zerogradient
                    else:
