...
def runExperiment():
    ...
    # plt.plot(epochs, accuracy, label='Validation Loss', marker='o')
    # plt.xlabel('Epochs')
    # plt.ylabel('Loss')
...
def make_local(dataset, data_split, label_split, federation):
    ...
    num_byzantine_users = int(np.ceil(cfg['byzantine_frac'] * num_active_users))
    ...
    for m in range(num_active_users):
        ...
        local[m] = Local(model_rate_m, data_loader_m, label_split[user_idx[m]])     # 为每个用户实例化一个Local对象
        if m in byzantine_user_idx:
            local[m].byzantine = True
...
class Local:
    ...
    def train(self, local_parameters, lr, logger):
        metric = Metric()                           # 实例化评估指标
        model = eval('models.{}(model_rate=self.model_rate).to(cfg["device"])'.format(cfg['model_name']))   # 动态加载模型
        model.load_state_dict(local_parameters)     # 加载本地模型参数
        model.train(True)
        optimizer = make_optimizer(model, lr)
        for local_epoch in range(1, cfg['num_epochs']['local'] + 1):
            for i, input in enumerate(self.data_loader):
                input = collate(input)      # 数据处理
                input_size = input['img'].size(0)                       # 获取批次的大小
                input['label_split'] = torch.tensor(self.label_split)   # 为输入数据添加标签划分
                input = to_device(input, cfg['device'])                 # 将数据迁移到指定设备
                optimizer.zero_grad()       # 清除梯度
                output = model(input)       # 模型前向传播
                output['loss'].backward()   # 计算损失的梯度
                if self.byzantine:
                    attack = 'random_noise'
                    for _, param in model.named_parameters():
                        if param.grad is not None:
                            if attack == 'random_noise':
                                update = torch.randn_like(param.grad)   # 正态分布
                                # update = torch.rand_like(param.grad)    # 均匀分布
                            elif attack == 'gradient_reversal':
                                k = 2.0     # 翻转倍数
                                update = -k * param.grad
                            elif attack == 'bias':
                                label_bias = []
                                update = param.grad + label_bias
                ...
                # torch.nn.utils.clip_grad_norm_(model.parameters(), 1)   # 梯度裁剪
                # optimizer.step()            # 更新模型参数
                evaluation = metric.evaluate(cfg['metric_name']['train']['Local'], input, output)   # 评估训练结果
                logger.append(evaluation, 'train', n=input_size)        # 记录评估结果
